...
it('renders simple message when there are less than 5 members registered', async () => {
  ...
  expect(reply()).toMatchInlineSnapshot(
    '"[@testuser](tg://user?id=128256) These members are registered as remote: [testuser_0](tg://user?id=0), [testuser_1](tg://user?id=1)"'
  );
});
...
it('renders no members message when there are no members registered', async () => {
  ...
  expect(reply()).toMatchInlineSnapshot(
    '"[@testuser](tg://user?id=128256) There are no members registered as remote."'
  );
});
...
it('pings all remote members', async () => {
  ...
  expect(reply()).toMatchInlineSnapshot(`
    "[@testuser](tg://user?id=128256) We selected 5 members registered as remote: [testuser_15345414](tg://user?id=15345414), [testuser_16775747](tg://user?id=16775747), [testuser_44098745](tg://user?id=44098745), [testuser_57984653](tg://user?id=57984653), [testuser_119209224](tg://user?id=119209224) 
